...
+ (NSString *)descriptionOfTopOfStack:(NSMutableArray *)stack
{
    ...
    else if([topOfStack isKindOfClass:[NSString class]])
    {
        ...
        if ([[ClacBrain typeOfString:topOfStack] isEqualToString:@"twoOperandOperation"])
        {
            NSString *second=[ClacBrain descriptionOfTopOfStack:stack];
            NSString *first=[ClacBrain descriptionOfTopOfStack:stack];
            description=[NSString stringWithFormat:@"(%@%@%@)",first,topOfStack,second];
            description=[ClacBrain surpressParienthese: description];
            //only two operand operation needs to surpress
        }
        if ([[ClacBrain typeOfString:topOfStack] isEqualToString:@"singleOperandOperation"]) {
            description=[NSString stringWithFormat:@"%@ ( %@ )",topOfStack,[ClacBrain descriptionOfTopOfStack:stack]];
        }
        if ([[ClacBrain typeOfString:topOfStack] isEqualToString:@"noOperandOperation"]) {
            description=[NSString stringWithFormat:@"%@",topOfStack];
        }
        if ([[ClacBrain typeOfString:topOfStack] isEqualToString:@"variable"]) {
...
+ (NSString *)surpressParienthese:(NSString *)description
{
    ...
    for (int i=[descriptionArray count]-1; i>0 && !lastOperation; i--) {
        if([ClacBrain isOperation:[descriptionArray objectAtIndex:i]]){
            lastOperation=[descriptionArray objectAtIndex:i];//last operation found
            
            for (int j=i-1; j>0 && !secondLastOperation; j--) {
                if ([ClacBrain isOperation:[descriptionArray objectAtIndex:j]]) {
                    secondLastOperation=[descriptionArray objectAtIndex:j];
                    
                }
            }
            if (![ClacBrain compareOperationPriority:lastOperation vs:secondLastOperation]) {
